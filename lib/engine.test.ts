...
const atto = resolveVehicle(C, { brandId: "byd", modelId: "byd-atto-3", year: 2024 }, now)!;
const oldPickup = resolveVehicle(C, { brandId: "isuzu", modelId: "isuzu-d-max", year: 2019 }, now)!;

describe("vehicle", () => {
  it("rejects unknown or out-of-range selections", () => {
    expect(resolveVehicle(C, { brandId: "toyota", modelId: "honda-city", year: 2025 }, now)).toBeNull();
    expect(resolveVehicle(C, { brandId: "byd", modelId: "byd-atto-3", year: 2015 }, now)).toBeNull();
  });
...
describe("quotes", () => {
  ...
  it("applies eligibility", () => {
    ...
    expect(quoteForProduct(C, "a-type1-dealer", oldPickup, now).status).toBe("ineligible");
  });
  it("prices from rules", () => {
    const q = quoteForProduct(C, "a-type1-dealer", corolla, now);
  ...
  it("snapshots are detached from the catalogue", () => {
    const q = generateQuotes(C, corolla, now)[0]!;
...
describe("matching", () => {
  it("ranks by priorities met, then premium", () => {
    const ranked = rankQuotes(generateQuotes(C, corolla, now), ["dealer", "noExcess", "replacementCar"], corolla);
  ...
  it("gives a reason for every priority", () => {
    const [top] = rankQuotes(generateQuotes(C, corolla, now), ["flood", "lowPremium"], corolla);
...
describe("compare", () => {
  it("explains why a cheaper plan is cheaper", () => {
    const quotes = generateQuotes(C, corolla, now);
  ...
  it("marks best values only when they differ", () => {
    const quotes = generateQuotes(C, corolla, now).filter((q) => q.coverage.insuranceType === "type1");
