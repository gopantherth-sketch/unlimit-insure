...
| P2.11 | Analytics and funnel events (no personal data) | Core | In progress | Built: anonymous counters + /admin/analytics. Live after D1 migration 0001 is applied (task C1). |
| P2.12 | Hosting on Cloudflare Workers + D1, domain, backups, monitoring | Core | In progress | Live: unlimit-insure.gopanther-th.workers.dev (D1 APAC). Pending: admin secrets, Git build commands in dashboard, custom domain, backups. |
...
| P2.16 | Multi-user admin: owner/staff accounts, lead assignment, per-user activity log | Core | Done | Live after D1 migrations 0001 + 0002 are applied (npm run db:migrate:remote). Break-glass ADMIN_USERNAME login kept. |
