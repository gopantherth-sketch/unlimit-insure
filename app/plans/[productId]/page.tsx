...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  const product = findProduct((await params).productId);
  return { title: product ? product.name : "ไม่พบแพ็กเกจ" };
}
...
export default async function PlanPage({ params, searchParams }: { params: Params; searchParams: Promise<RawParams> }) {
  ...
  const result = vehicle ? quoteForProduct(productId, vehicle) : null;
  const quote = result?.status === "ok" ? result.quote : null;
  const match = quote && vehicle && input ? matchQuote(quote, input.priorities, buildMatchContext(generateQuotes(vehicle), vehicle)) : null;
